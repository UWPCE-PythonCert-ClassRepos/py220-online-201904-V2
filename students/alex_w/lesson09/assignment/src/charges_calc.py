...
def parse_cmd_arguments():
    ...
    parser.add_argument('-c', '--conditional',
                        help='conditional logging (0=False, 1=True)', required=True)
...
def conditional_logging(func):
    ...
    def wrapper(*args, **kwargs):
        global LOGGER_HANDLE
        global CONDITIONAL_LOGGING
        original_setting = LOGGER_HANDLE.disabled
        if CONDITIONAL_LOGGING:
            LOGGER_HANDLE.disabled = True
        func(*args, **kwargs)
        LOGGER_HANDLE.disabled = original_setting
    return wrapper
