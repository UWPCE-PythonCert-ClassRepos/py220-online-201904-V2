...
formatter = logging.Formatter('%(asctime)s:%(filename)s:%(lineno)-4ds:%(levelname)s:%(message)s')
file_handler = logging.FileHandler('db.log')
file_handler.setFormatter(formatter)
logger.addHandler(file_handler)

logging.basicConfig(level=logging.DEBUG, filename='db.log',
                    format='%(asctime)s:%(filename)s:%(lineno)-4ds:%(levelname)s:%(message)s')
...
def add_tables():
    ...
    logging.INFO('Customer table is created.')
    ...
    with open(DATABASE_CSV_FILE, 'r', encoding='iso-8859-1') as f:
        for line in f.readlines()[1:]:
            ...
            if len(record) == 8:
                ...
                db_record.save()
    logger.debug("Finished. %s" % datetime.datetime.now().strftime("%H:%M:%S"))
...
def search_customer(customer_id):
    ...
    """List Comphension returning Search customer record"""
...
def list_active_customers():
    ...
    """List Comphension returning Active customers records"""
...
def name_gen(letter):
    """ Genearator that returns first names that start with 'letter' """
    records = Customer.select().where(Customer.status == 'Active')
    for rec in records:
        if rec.first_name.startswith(letter):
            yield rec.first_name
...
def main():
    ...
    logger.debug(search_customer('C000008'))
    logger.debug(search_customer('C000009'))
    ...
    """ Print all customer first names that start with 'G'"""
    first_name_gen = name_gen('G')
    print("--- Names starting with 'G' ---")
    for name in first_name_gen:
        logger.debug(name)
    print()
