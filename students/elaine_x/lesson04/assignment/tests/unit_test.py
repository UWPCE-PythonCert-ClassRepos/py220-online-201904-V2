...
def _add_customers():
    return [
        ...
        ("124", "Name", "Lastname", "Address", "phone", "email", "active", 999),
...
def test_search_customer(_search_customers):
    ...
    assert result["lastname"] == (_search_customers[1][2])
    assert result["email"] == (_search_customers[1][5])
...
def test_update_customer_credit(_update_customer_credit):
    ...
    with pytest.raises(Exception) as excinfo:
        l.update_customer_credit("00100", 500) # error
