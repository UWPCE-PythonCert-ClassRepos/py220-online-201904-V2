...
def main():
    ...
    with mongo:
        ...
        print("Collecting cd collector info")
        for person in collector.find():
            cds_for_collector = cd.aggregate([
                {'$match': {'name': person['name']}}
                ])
            for cd in cds_for_collector:
                print(f"{cd['name']} collected a cd: {cd['artist']},"
                        f"{cd['Title']}")
